...
func BulletPreSolve(arb *cp.Arbiter, _ *cp.Space, data interface{}) bool {
	...
	case *tanklets.Tank:
		...
		if !tank.Destroyed {
			...
			game.Network.Players.SendAll(game.Network, pkt.Damage{tank.ID, bullet.PlayerID})
			...
			case len(tanksAlive) == 1:
				...
				game.Network.Players.SendAll(game.Network, pkt.State{State: tanklets.StateWinCountdown, ID: tanksAlive[0].ID})
			case len(tanksAlive) == 0:
				game.EndTime = time.Now()
				game.State = tanklets.StateFailCountdown
				game.Network.Players.SendAll(game.Network, pkt.State{State: tanklets.StateFailCountdown})
			}
		}
	...
	case *tanklets.Bullet:
		...
		game.Network.Players.SendAll(game.Network, shot1, shot2)
	default:
		...
		game.Network.Players.SendAll(game.Network, shot)
	}
	return false
}
